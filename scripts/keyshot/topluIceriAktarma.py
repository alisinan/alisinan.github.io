...
def checkDialogOptions(opts, importExtension):
  ...
  for file in os.listdir(opts["ifold"]):
    #Condition: ".sldasm" matches file in import folder.
  ...
  if not found:
    #Script stops with error and shows exception in "Show Output". No files with extension "sldasm" (importExtension) in import folder.
    raise Exception("Could not find any sldasm files in \"{}\"!".format(opts["ifold"]))

#Function to set the right import options for sldasm models.
...
def getCameras(allCameras):
  #Condition: "Render all cameras" is checked in dialog.
  if allCameras:
    #Put the cameras in the scene in a list. [0:-1] to exclude the 'default' camera.
    cameraToRender = lux.getCameras()[0:-1]
...
def main():
  ...
  values = [("ifold", lux.DIALOG_FOLDER, "Folder to import sldasm models from:", None),
            (lux.DIALOG_LABEL, "--"),
            ...
            ("toon", lux.DIALOG_CHECK, "Include Toon pass", True),
            (lux.DIALOG_LABEL, "--"),
  ...
  opts = lux.getInputDialog(title = "Batch Render Model Collection",
                            ...
                            id = "batchrendersldasmmodels.py.luxion")
  if not opts: return

  #Set sldasm as the model format to import.
  importExtension = "sldasm"
  ...
  for file in os.listdir(importFolder):
    #Condition: file ends with extension "importExtension" ("sldasm").
    if file.endswith(importExtension):
      ...
      lux.setMaterialTemplate(name = "Toy Train - Realistic")
      style = "Realistic"
      ...
      #Condition: "Include Toon pass" is checked in dialog.
      if toonPass:
        #Apply material template for "Toon" style to scene.
        lux.setMaterialTemplate(name = "Toy Train - Toon")
        style = "Toon"
        #Call function to render images and pass the necessary arguments. (Find function above main())
        renderImage(cameraToRender, outputFolder, file, style, outputExtension, imageWidth, imageHeight, renderOptions)
